...
출력 과정인 읽기나 쓰기 과정이 수행될 때 코드가 더 이상 진행되지 않고 내부 메모리 영역에서 해당 작업이 마무리가 될 때까지 코드는 멈추게 되며, 이런상황을 '블로킹'하고 있다라고 한다.<br>
예를 들어 쓰기 과정에서 쓰려고 시도하는데 기존의 공간이 꽉 차 있다면 공간이 빌 때까지 쓸 수가 없으므로 블로킹한다. 

## 넌블로킹 동작
코드의 흐름을 멈추지 않고 다른 루틴을 먼저 수행할 수 있기 때문에 성능이나 실행 시간에 좀더 좋은 성능을 보여준다.
